...
def get_vid():
    ...
    if not link or not directory:
        print("❌ Missing link or folder")
        return

    output_template = f"{directory}/%(title)s.%(ext)s"

    if format_selected == "MP4":
        command = [yt_dlp_path, "-f", "bestvideo+bestaudio", "-o", output_template, link]
    ...
    try:
        ...
        messagebox.showinfo(title="Confirmation", message="Download Completed!!")
    except subprocess.CalledProcessError as e:
        messagebox.showwarning(title="Warning", message="Download Failed!!")
